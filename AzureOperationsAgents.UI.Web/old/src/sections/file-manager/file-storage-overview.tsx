...
export default function FileStorageOverview({ data, total, chart, ...other }: Props) {
  ...
  const chartOptions = useChart({
    ...
    plotOptions: {
      radialBar: {
        ...
        dataLabels: {
          ...
          total: {
            label: `${t('analytics.used-of')} ${total} GB`,
    ...
    fill: {
      ...
      gradient: {
        colorStops: [
          { offset: 0, color: colors[0], opacity: 0.15 }, // Add opacity
          { offset: 100, color: colors[1], opacity: 0.7 }, // Add opacity
        ],
      },
    },
    ...options,
        ...
        {data.map((category) => (
          <Stack key={category.name} spacing={2} direction="row" alignItems="center">
            ...
            <ListItemText
              ...
              secondary={`${category.filesCount} ${t('analytics.files-word')}`}

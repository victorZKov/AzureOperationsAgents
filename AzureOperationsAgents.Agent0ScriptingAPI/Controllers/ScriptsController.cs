...
namespace AzureOperationsAgents.Agent0ScriptingAPI.Controllers;
...
public class ScriptsController : ControllerBase
{
    ...
    public ScriptsController(IScriptGenerationService scriptGenerationService, IScriptRepository scriptRepository)
    {
    ...
    [HttpPost]
    public async Task<ActionResult<Script>> GenerateScript([FromBody] GenerateScriptRequest request)
    {
        try
        {
    ...
    [HttpPost("{id}/refine")]
    public async Task<ActionResult<Script>> RefineScript(Guid id, [FromBody] RefineScriptRequest request)
    {
        var script = await _scriptRepository.GetByIdAsync(id);
        if (script == null)
            return NotFound();

        var refinedScript = await _scriptGenerationService.RefineScriptAsync(script, request.Feedback);
